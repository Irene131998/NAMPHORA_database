...
abstract: > 
 North Africa’s climate and vegetation have undergone major shifts, particularly during the African Humid Period (AHP). Fossil pollen records are crucial for reconstructing past vegetation and climate, offering insights into the origin and distribution of past flora. However, there are still gaps in the existing pollen databases, as some unpublished data have yet to be recorded, hindering further progress in the reconstruction of the AHP landscape. Thus, we introduce the Northern African, Arabian, and Mediterranean Pollen Holocene Records Archive (NaAMPHoRA)—a comprehensive, machine-readable database compiling fossil and modern pollen records alongside plant functional traits (PFTs), phytogeographical data, and taxonomic harmonisation. It enables direct data retrieval via programming languages such as R, and all datasets and code are openly available via GitHub, with contributions and feedback welcomed. This database will be an important resource for research on palaeoecology, biogeography and climate modelling during the Holocene.
keywords:
  - Pollen records
...
-   `data/`: All raw and processed data are stored in this folder.
    ...
    -   `/raw_data`: Contains raw data on the following sub-folders:
        ...
        -   `/mapping_data`: Contains the WWF terrestrial ecoregions of
            the world shapefile in the folder `/EcoregionsWWF_2017`
            (retrieved from
            https://www.worldwildlife.org/publications/terrestrial-ecoregions-of-the-world),
            the phytoregions defined by White (1983) in the folder
            `/Africa_Vegetation_White_1983` (retrieved from
...
-   `docs/`
    ...
    -   `/references`: Contains the *pollen_records_references.bib,
        phytogeographic_affinity_references.bib, habit_references.bib,
        pfts_references.bib & taxonomy_references.bib* files.
...
-   `metadata/`
    ...
    -   `/references`: Contains the *database_references.csv* file,
        ...
        data, taxonomy, habit, phytogeographic affinity and plant
        functional traits).
