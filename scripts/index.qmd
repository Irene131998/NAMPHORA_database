---
title: "NAMPHORA: An Open-access Database of Fossil and Modern Pollen Records from Northern Africa and adjacent Mediterranean and Arabian regions"
author:
...
-   `data/`: All raw and processed data are stored in this folder.
    ...
    -   `/processed_data:` Contains processed data on the following
        ...
        -   `/pollen_data`: Contains harmonised fossil (`/fossil`
            folder) and modern (`/modern` folder) pollen records, with
            each data file containing a column listing the reference(s)
            for the source publication(s). Each folder contains:
...
-   `metadata/`
    ...
    -   `/age_calibrated`: Contains the following:
        ...
        -   *calibration_information.csv:* A file that provides details
            on the calibration process for each record, and the sources
            where the chronological data originates from.
    ...
    -   `/pollen_data`: Contains the following:
        ...
        -   *pollen_data_received_from_authors_information.csv*: This
            file provides details on pollen records received from
            authors, such as contact details, name of researcher and its
            organisation and the method of data collection.
    ...
    -   `/references`: Contains the *database_references.csv* file,
        ...
        functional traits), and the
        *Contributing_data_authors_list.docx* file, which includes a
        list with all contributing authors who provided data to the
        database.
...
`11_illustrative_examples_database_use.R` (Shows how to use the database
with an example for a single taxon and another example for a pollen
record)
